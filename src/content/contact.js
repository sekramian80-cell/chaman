/**
 * اطلاعات تماس و CTA
 */

export const contactInfo = {
  phone: '+989120000000',
  phoneDisplay: '۰۹۱۲ ۰۰۰ ۰۰۰۰',
  instagram: 'https://instagram.com/',
  location: 'تهران و کرج، اجرای پروژه در سراسر کشور',
  whatsapp: 'https://wa.me/989120000000',
};

export const contactCTA = {
  eyebrow: 'شروع پروژه',
  title: 'برای فضای شما چه مدل چمنی مناسب‌تر است؟',
  description:
    'متراژ، محل اجرا و چند عکس از فضا را بفرستید تا پیشنهاد اولیه و بازه قیمت دقیق‌تری دریافت کنید.',
  primaryCta: {
    label: 'تماس مستقیم',
    icon: 'Phone',
    href: 'tel:+989120000000',
  },
  secondaryCta: {
    label: 'پیام در واتساپ',
    icon: 'MessageCircle',
    href: 'https://wa.me/989120000000',
  },
};

export const pageHeros = {
  services: {
    eyebrow: 'صفحه خدمات',
    title: 'خدمات چمن مصنوعی برای هر نوع فضا',
    description:
      'در این بخش نمونه خدمات دمو را می‌بینید؛ از مشاوره و انتخاب مدل تا زیرسازی، نصب و نگهداری.',
    primaryLabel: 'درخواست مشاوره',
    secondaryLabel: 'دیدن نمونه اجرا',
  },
  products: {
    eyebrow: 'صفحه محصولات',
    title: 'مدل‌های پیشنهادی چمن مصنوعی',
    description:
      'در این صفحه چند مدل دمو برای مقایسه ظاهر، تراکم، دوام و کاربردهای مختلف قرار گرفته است.',
    primaryLabel: 'استعلام قیمت',
    secondaryLabel: 'مشاهده پروژه‌ها',
  },
  process: {
    eyebrow: 'صفحه روند اجرا',
    title: 'روند همکاری سریع، شفاف و مرحله‌به‌مرحله',
    description:
      'این صفحه برای توضیح دمو از مسیر اجرای پروژه طراحی شده تا مشتری بداند از اولین تماس تا تحویل چه اتفاقی می‌افتد.',
    primaryLabel: 'شروع پروژه',
    secondaryLabel: 'دیدن نمونه اجرا',
  },
  projects: {
    eyebrow: 'صفحه نمونه کار',
    title: 'نمونه پروژه‌های دمو برای نمایش کیفیت اجرا',
    description:
      'در این بخش چند سناریوی پیشفرض از پروژه‌ها، متراژها و کاربردهای رایج قرار گرفته تا صفحه نمونه‌کار کامل‌تر باشد.',
    primaryLabel: 'درخواست بازدید',
    secondaryLabel: 'مشاهده خدمات',
  },
  faq: {
    eyebrow: 'صفحه سوالات',
    title: 'پاسخ سوالات رایج قبل از خرید و نصب',
    description:
      'اینجا سوالات پرتکرار به شکل دمو دسته‌بندی شده‌اند تا تصمیم‌گیری درباره مدل، قیمت و نگهداری ساده‌تر شود.',
    primaryLabel: 'پرسیدن سوال',
    secondaryLabel: 'دیدن محصولات',
  },
};
