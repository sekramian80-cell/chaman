<?php
/**
 * فراز چمن — مدیریت محصولات (افزودن، لیست، ویرایش، حذف)
 *
 * نحوه نصب:
 * 1. Code Snippets → Snippet «فراز چمن — افزودن محصول» را باز کن
...
function faraz_get_flat_subcategories()
{
    $flat = [];

    foreach (faraz_get_product_subcategories_tree() as $parent => $items) {
        foreach ($items as $item) {
...
add_action('admin_menu', function () {
    ...
    add_submenu_page(
        ...
        'مدیریت محصولات',
        'edit_posts',
        'faraz-manage-products',
        'faraz_render_products_list_page'
    );
    ...
    add_submenu_page(
        ...
        'edit_posts',
        'faraz-edit-product',
        'faraz_render_edit_product_page'
    );
}, 30);

add_action('admin_enqueue_scripts', function ($hook) {
    if (!faraz_is_product_admin_page($hook)) {
        return;
    }
    ...
    if (in_array($hook, ['product_page_faraz-add-product', 'product_page_faraz-edit-product'], true)) {
        wp_enqueue_media();
        wp_register_script('faraz-product-admin', false, ['jquery'], '1.2.0', true);
        wp_enqueue_script('faraz-product-admin');
        wp_add_inline_script('faraz-product-admin', faraz_product_form_js());
        wp_localize_script('faraz-product-admin', 'farazProductSubcategories', faraz_get_product_subcategories_tree());
    }

    if ($hook === 'product_page_faraz-manage-products') {
        wp_register_script('faraz-product-list', false, ['jquery'], '1.2.0', true);
        wp_enqueue_script('faraz-product-list');
        wp_add_inline_script('faraz-product-list', faraz_product_list_js());
    }
});

add_action('admin_post_faraz_save_product', 'faraz_handle_save_product');
add_action('admin_post_faraz_update_product', 'faraz_handle_update_product');
add_action('admin_post_faraz_delete_product', 'faraz_handle_delete_product');
...
function faraz_handle_save_product()
{
    ...
    $form = faraz_parse_product_form_request();
    if (is_wp_error($form)) {
        faraz_redirect_with_notice('error', $form->get_error_message(), 'faraz-add-product');
    }
    ...
    if (is_wp_error($post_id)) {
        faraz_redirect_with_notice('error', $post_id->get_error_message(), 'faraz-add-product');
    }

    faraz_persist_product_data((int) $post_id, $form);

    $message = $status === 'publish' ? 'محصول با موفقیت ثبت شد.' : 'محصول به‌صورت پیش‌نویس ذخیره شد.';
    faraz_redirect_with_notice('success', $message, 'faraz-manage-products');
}

function faraz_handle_update_product()
{
    if (!current_user_can('edit_posts')) {
        wp_die('دسترسی مجاز نیست.');
    }

    check_admin_referer('faraz_update_product');

    $post_id = absint($_POST['product_id'] ?? 0);
    $post = faraz_get_product_post($post_id);

    if (!$post) {
        faraz_redirect_with_notice('error', 'محصول یافت نشد.', 'faraz-manage-products');
    }

    $form = faraz_parse_product_form_request();
    if (is_wp_error($form)) {
        faraz_redirect_with_notice('error', $form->get_error_message(), 'faraz-edit-product', ['product_id' => $post_id]);
    }
    ...
    if (is_wp_error($updated)) {
        faraz_redirect_with_notice('error', $updated->get_error_message(), 'faraz-edit-product', ['product_id' => $post_id]);
    }

    faraz_persist_product_data($post_id, $form);

    $message = $status === 'publish' ? 'تغییرات محصول ذخیره شد.' : 'محصول به‌صورت پیش‌نویس ذخیره شد.';
    faraz_redirect_with_notice('success', $message, 'faraz-manage-products');
}

function faraz_handle_delete_product()
{
    ...
    check_admin_referer('faraz_delete_product_' . $post_id);
    ...
    if (!$deleted) {
        faraz_redirect_with_notice('error', 'حذف محصول انجام نشد.', 'faraz-manage-products');
    }

    faraz_redirect_with_notice('success', 'محصول به سطل زباله منتقل شد.', 'faraz-manage-products');
}

function faraz_render_add_product_page()
{
    faraz_render_product_form([
        'mode' => 'add',
        'values' => faraz_get_empty_product_form_values(),
        ...
        'submit_label' => 'افزودن و انتشار محصول',
        'action' => 'faraz_save_product',
        'nonce_action' => 'faraz_save_product',
        'back_url' => faraz_admin_page_url('faraz-manage-products'),
    ]);
}

function faraz_render_edit_product_page()
{
    $post_id = absint($_GET['product_id'] ?? 0);
    $post = faraz_get_product_post($post_id);

    if (!$post) {
        echo '<div class="wrap"><div class="notice notice-error"><p>محصول یافت نشد.</p></div></div>';
        return;
    }

    faraz_render_product_form([
        'mode' => 'edit',
        'values' => faraz_get_product_form_values($post_id),
        ...
        'action' => 'faraz_update_product',
        'nonce_action' => 'faraz_update_product',
        'back_url' => faraz_admin_page_url('faraz-manage-products'),
        'product_id' => $post_id,
    ]);
}

function faraz_render_products_list_page()
{
    if (!current_user_can('edit_posts')) {
        return;
    }
    ...
    $products = faraz_get_products_for_list();
    $main_categories = faraz_get_main_categories();
    ?>
    <div class="wrap faraz-product-admin" dir="rtl">
        <div class="faraz-list-header">
            <div>
                <h1>مدیریت محصولات</h1>
                <p class="description">لیست همه محصولات با فیلتر زیردسته. ویرایش و حذف از همین صفحه انجام می‌شود.</p>
            </div>
            <a class="button button-primary" href="<?php echo esc_url(faraz_admin_page_url('faraz-add-product')); ?>">افزودن محصول جدید</a>
        </div>

        <?php faraz_render_admin_notice($notice, $message); ?>
        ...
        <div class="faraz-card faraz-list-card">
            <div class="faraz-tabs" role="tablist" aria-label="فیلتر محصولات">
                ...
                <?php foreach (faraz_get_flat_subcategories() as $item) : ?>
                    <button
                        type="button"
            ...
            <div class="faraz-table-wrap">
                <table class="widefat fixed striped faraz-products-table">
                    <thead>
                        <tr>
                            ...
                            <?php foreach ($products as $product) : ?>
                                ...
                                >
                                    ...
                                    <td class="col-actions">
                                        ...
                                        <a
                                            class="button button-small button-link-delete faraz-delete-product"
...
function faraz_render_product_form($config)
{
    if (!current_user_can('edit_posts')) {
        return;
    }
    ...
    wp_localize_script('faraz-product-admin', 'farazProductFormInit', [
    ...
    <div class="wrap faraz-product-admin" dir="rtl">
        ...
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="faraz-product-form">
            ...
            <?php if ($is_edit) : ?>
                <input type="hidden" name="product_id" value="<?php echo esc_attr((int) $config['product_id']); ?>" />
            <?php endif; ?>
            ...
            <div class="faraz-grid">
                <div class="faraz-main">
                    <div class="faraz-card">
                        <h2>اطلاعات اصلی</h2>
                        <p>
                            <label for="product_title"><strong>نام محصول</strong> <span class="required">*</span></label>
                            <input type="text" class="large-text" id="product_title" name="product_title" required value="<?php echo esc_attr($values['title']); ?>" />
                        </p>
                        <p>
                        ...
                        <p>
                            <label for="product_content"><strong>محتوای اصلی محصول</strong></label>
                            <?php
                            wp_editor($values['content'], 'product_content', [
                ...
                <div class="faraz-side">
                    <div class="faraz-card">
                        ...
                        <p>
                            ...
                            <?php foreach ($categories as $slug => $label) : ?>
                                <label class="faraz-radio">
                                    <input type="radio" name="product_category" value="<?php echo esc_attr($slug); ?>" <?php checked($values['primary_category'], $slug); ?> required />
                                    <?php echo esc_html($label); ?>
                                </label>
                            <?php endforeach; ?>
                        </p>
                        <p>
                            <label for="product_subcategory"><strong>زیردسته</strong> <span class="required">*</span></label>
                        ...
                        <p>
                            <label for="product_price"><strong>قیمت</strong></label>
                            <input type="text" class="regular-text" id="product_price" name="product_price" value="<?php echo esc_attr($values['product_price']); ?>" />
                        </p>
                        <p>
...
function faraz_parse_product_form_request()
{
    $title = sanitize_text_field(wp_unslash($_POST['product_title'] ?? ''));
    if ($title === '') {
        return new WP_Error('faraz_missing_title', 'نام محصول الزامی است.');
    }
    ...
    if (!in_array($category, $main_categories, true)) {
        return new WP_Error('faraz_invalid_category', 'دسته اصلی محصول معتبر نیست.');
    }
    ...
    if (!in_array($subcategory, $allowed_subcategories, true)) {
        return new WP_Error('faraz_invalid_subcategory', 'زیردسته محصول را انتخاب کنید.');
    }
...
function faraz_persist_product_data($post_id, $form)
{
    ...
    faraz_assign_product_terms($post_id, $form['category'], $form['subcategory']);
    faraz_update_product_meta($post_id, 'short_description', $form['short_description']);
    faraz_update_product_meta($post_id, 'project_meta', $form['project_meta']);
    faraz_update_product_meta($post_id, 'product_price', $form['product_price']);
    faraz_update_product_meta($post_id, 'seo_title', $form['seo_title']);
    faraz_update_product_meta($post_id, 'seo_description', $form['seo_description']);
    faraz_update_product_meta($post_id, 'robots_noindex', $form['allow_index'] ? '0' : '1');
    faraz_update_product_meta($post_id, 'gallery', implode(',', $form['gallery_ids']));
    faraz_update_product_meta($post_id, 'subcategory', $form['subcategory']);

    if (function_exists('update_field')) {
        update_field('short_description', $form['short_description'], $post_id);
        update_field('project_meta', $form['project_meta'], $post_id);
        update_field('product_price', $form['product_price'], $post_id);
...
function faraz_get_empty_product_form_values()
{
    return [
        'title' => '',
        'short_description' => '',
...
function faraz_get_product_form_values($post_id)
{
    ...
    return [
        ...
        'short_description' => faraz_get_product_meta($post_id, 'short_description') ?: ($post->post_excerpt ?? ''),
        'content' => $post ? $post->post_content : '',
        'project_meta' => faraz_get_product_meta($post_id, 'project_meta'),
        'product_price' => faraz_get_product_meta($post_id, 'product_price'),
        'seo_title' => faraz_get_product_meta($post_id, 'seo_title'),
        'seo_description' => faraz_get_product_meta($post_id, 'seo_description'),
        'slug' => $post ? $post->post_name : '',
        'primary_category' => faraz_get_product_primary_category_slug($post_id) ?: 'sports',
        'subcategory' => faraz_get_product_subcategory_slug($post_id),
        'featured_image_id' => $featured_id,
        'gallery_ids' => faraz_get_product_gallery_ids($post_id),
        'robots_index' => faraz_get_product_meta($post_id, 'robots_noindex') !== '1',
    ];
}

function faraz_get_products_for_list()
{
    $posts = get_posts([
        'post_type' => 'product',
    ...
    foreach ($posts as $post) {
        $id = (int) $post->ID;
        $primary = faraz_get_product_primary_category_slug($id);
        $subcategory = faraz_get_product_subcategory_slug($id);
        ...
        $rows[] = [
            ...
            'project_meta' => faraz_get_product_meta($id, 'project_meta'),
            'price' => faraz_get_product_meta($id, 'product_price'),
            'primary_category' => $primary,
            'primary_label' => $main_categories[$primary] ?? '—',
            'subcategory' => $subcategory,
            'subcategory_label' => $subcategory ? faraz_get_subcategory_label($subcategory) : '—',
            'status' => $status,
            'status_label' => faraz_get_status_label($status),
            'thumb' => get_the_post_thumbnail_url($id, 'thumbnail') ?: '',
            'edit_url' => faraz_admin_page_url('faraz-edit-product', ['product_id' => $id]),
            'delete_url' => wp_nonce_url(
                admin_url('admin-post.php?action=faraz_delete_product&post_id=' . $id),
                'faraz_delete_product_' . $id
            ),
        ];
    }
...
function faraz_get_status_label($status)
{
    $labels = [
        'publish' => 'منتشر شده',
        'draft' => 'پیش‌نویس',
...
function faraz_get_product_post($post_id)
{
...
function faraz_is_product_admin_page($hook)
{
    return in_array($hook, [
        'product_page_faraz-manage-products',
        'product_page_faraz-add-product',
        'product_page_faraz-edit-product',
    ], true);
}

function faraz_admin_page_url($page, $args = [])
{
    return add_query_arg(array_merge(['page' => $page], $args), admin_url('edit.php?post_type=product'));
}

function faraz_render_admin_notice($notice, $message)
{
...
function faraz_redirect_with_notice($type, $message, $page = 'faraz-manage-products', $args = [])
{
    $redirect = add_query_arg(array_merge([
        'page' => $page,
        'faraz_notice' => $type,
        'faraz_message' => rawurlencode($message),
    ], $args), admin_url('edit.php?post_type=product'));
...
{
    return '
        .faraz-product-admin .description { margin-bottom: 0; }
        ...
        .faraz-products-table .col-thumb { width: 72px; }
        .faraz-products-table .col-actions { width: 150px; }
...
function faraz_product_form_js()
{
    return <<<'JS'
jQuery(function ($) {
    let featuredFrame;
    let galleryFrame;
    let galleryIds = (window.farazProductFormInit && window.farazProductFormInit.galleryIds) || [];
    ...
    function renderSubcategories() {
        ...
        const preferred = (window.farazProductFormInit && window.farazProductFormInit.subcategory) || $select.val();
    ...
    if (window.farazProductFormInit) {
        const init = window.farazProductFormInit;
        $('input[name="product_category"][value="' + init.primaryCategory + '"]').prop('checked', true);
    ...
    $('#product_title').on('input', function () {
        ...
        if ((window.farazProductFormInit && window.farazProductFormInit.lockSlug) || $slug.val().trim() !== '') return;
...
function faraz_product_list_js()
{
    return <<<'JS'
jQuery(function ($) {
    function applyFilter(filter) {
        let visible = 0;
        $('.faraz-products-table tbody tr').not('.faraz-empty-row').each(function () {
            const $row = $(this);
    ...
    $('.faraz-delete-product').on('click', function (e) {
